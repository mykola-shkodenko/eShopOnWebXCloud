...
resource "azurerm_service_plan" "windows-eus" {
  name                = "asp-eshop-cloudx-eus"
...
resource "azurerm_application_insights" "web-eus" {
  name                = "appi-web-eshop-cloudx-eus"
  location            = azurerm_service_plan.windows-eus.location
  resource_group_name = azurerm_resource_group.this.name
  application_type    = "web"
}

# Create web app for Web
resource "azurerm_windows_web_app" "web-eus" {
  name                = "${var.app_web_name}-eus"
  resource_group_name = azurerm_resource_group.this.name
  location            = azurerm_service_plan.windows-eus.location
  service_plan_id     = azurerm_service_plan.windows-eus.id
...
data "azuread_service_principal" "web-eus" {
  ...
  depends_on = [
    azurerm_windows_web_app.web-eus
  ]
}
resource "azurerm_key_vault_access_policy" "web-eus" {
  key_vault_id = azurerm_key_vault.kv.id
  tenant_id    = data.azurerm_client_config.current.tenant_id
  object_id    = data.azuread_service_principal.web-eus.object_id
